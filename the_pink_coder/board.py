...
class Board:
    ...
    def update_available_throw(self):
        ...
        # Updating the avaliable throw choice 
        # based on it is upper player or lower player
    ...
    def generate_best_action(self, multi=False):
        ...
        possible_ally_actions = self.get_ally_actions()
        possible_oppo_actions = self.get_oppo_actions()
    ...
    def evaluation(self, old_board):   
        ...
        # Assign weight to different feature 
    ...
    # Generate the potential good throw of ally, which could kill opponent directly
    def direct_ally_throws(self):
    ...
    # Generate the potential great action for opponent, which to kill tha ally directly
    def direct_oppo_throws(self):
        ...
        if self.oppo_throw_remain > 2:
            for piece in self.board[self.ally]:
                if piece[1] in self.possible_oppo_throws:
                    if (piece[0] == 'r' and (ally_dict["p"]+1) /(len(self.board[self.oppo])+1) <= 0.6) or (piece[0] == 'r' and remain_oppo <= 3):
                        possible_ally_actions.append(("THROW", "p", (piece[1])))
                    elif (piece[0] == 'p' and (ally_dict["s"]+1) /(len(self.board[self.oppo])+1) <= 0.6) or (piece[0] == 'p' and remain_oppo <= 3):
                        possible_ally_actions.append(("THROW", "s", (piece[1])))
                    elif (piece[0] == 's' and (ally_dict["r"]+1) /(len(self.board[self.oppo])+1) <= 0.6) or (piece[0] == 's' and remain_oppo <= 3):
                        possible_ally_actions.append(("THROW", "r", (piece[1])))
    ...
    def get_ally_actions(self):
        
        possible_throw_actions = []
        possible_move_actions = []

        all_action = self.direct_ally_throws()
        if all_action != []:
            return all_action

        throw_remain = self.ally_throw_remain
        possible_move_actions = self.get_ally_moves()
    ...
    def get_oppo_actions(self):
        
        possible_throw_actions = []
        possible_move_actions = []

        all_action = self.direct_oppo_throws()
        if all_action != []:
            return all_action

        throw_remain = self.oppo_throw_remain
        possible_move_actions = self.get_oppo_moves()
